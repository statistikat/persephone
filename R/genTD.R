...
genTD <- function(ff = 12, fYear = 1960, lYear = 2099, ft, ww){
  ...
  preDef <- listHolidays()
  #ft <- list("NewYearsDay","Epiphany","EasterMonday","LaborDay","PentecostMonday","Ascension",
  #  "CorpusChristi","AssumptionOfMary","10-26","AllSaints","ITImmaculateConception","ChristmasEve",
  ...
  dat <- lapply(ft,function(x){
    if(substr(x,1,7)=="easter+"){
      ...
      a <- as.data.table(as.Date(paste0(seq(start(y)[1],end(y)[1]),paste0("-",x))))
    }
  })
  names(dat) <- unlist(ft)
