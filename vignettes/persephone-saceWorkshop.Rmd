...
All predefined JDemetra+ model specifications can be used in the calls to per_x13 or per_tramo (and also in the hierachical functions), but most of the times it is still necessary to adjust parameters of seasonal adjustment steps more individual. This can of course be done.
...
w_non_EA19 <- weights_pi_eu28$weight[
   which(!weights_pi_eu28$country %in% weights_pi_ea19$country)]
# The first per_hts call is for the EA19 countries
...
```
Please note that in the generation step of `hts_EU28` the weights in `w_non_EA19` are only used for the single persephone objects in the `list` input and not for the hierachical persephone object `hts_EA19`.
...
with the exception that no overall index of quality is included since it is not available to us from **RJDemetra** at the moment and is not trivial to recalculate it.
