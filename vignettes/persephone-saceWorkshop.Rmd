...
It builds on top of **RJDemetra** (\url{https://github.com/jdemetra/rjdemetra}), an R interface to JDemetra+ (https://github.com/jdemetra), and provides analytic tools such as interactive plots to support the SA expert. 
...
Depending on whether seasonal adjustment should performed with X-13 or with TRAMO-SEATS,
...
```{r}
data(AirPassengers, package = "data sets")
...
In the examples below, we use time series of the volume index of production for total industry (except construction) 
...
Load the package's internal data sets.
...
We remove the last time instance because of the missing value.
...
We construct a hierarchical persephone object that does not contain weights and returns aggregate 
series as the sum of the components.

To get started, we generate persephone objects with x13 as method for all countries’ time series.
